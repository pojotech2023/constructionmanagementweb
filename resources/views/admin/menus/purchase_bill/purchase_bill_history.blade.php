...
<div class="container">
    <div class="row align-items-center">
        <div class="col-lg-8 offset-lg-1">
            <h3 class="pb-4 mt-3">Purchase Bill History — {{ $site->site_name }}</h3>
        </div>
        <div class="col-lg-2 text-end pb-4">
            <a href="{{ route('purchaseBill.form', $site->id) }}" class="btn btn-primary">+ New Bill</a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-11">
            <div class="card shadow-lg p-4 ms-4">
                ...
                @if ($bills->isEmpty())
                    <p class="text-center mt-3">No purchase bills generated for this site yet.</p>
                @else
                        ...
                        <table class="table table-bordered table-striped align-middle">
                            ...
                            <tbody>
                                @foreach ($bills as $index => $bill)
                                    @php
                                        $pdfUrl = asset('storage/purchase_bills/purchase_bill_' . $bill->id . '.pdf');
                                        $waMessage = urlencode("Hi {$bill->name}, your purchase bill is ready. Download here: $pdfUrl");
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                ...
                                                <button type="button" class="btn btn-sm btn-danger deleteBillBtn"
                                                    data-url="{{ route('purchaseBill.delete', $bill->id) }}"
                                                    data-name="the purchase bill for {{ $bill->name }} ({{ \Carbon\Carbon::parse($bill->date)->format('d-m-Y') }})"
