...
@section('content')
                ...
                <div class="col-md-12">
                    <div class="card payment-history-card">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            ...
                            <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#vendorPaymentExportModal">
                                Export Excel
                            </button>
                        </div>
    ...
    <div class="modal fade" id="vendorPaymentExportModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form method="GET" action="{{ route('payment.history.export', ['vendorId' => $vendorId]) }}" class="js-export-modal-form">
                    <div class="modal-header">
                        <h5 class="modal-title">Export Vendor Payment History</h5>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.editPaymentBtn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    form.action = "{{ route('payment.update', ':id') }}".replace(':id', id);
            ...
            document.getElementById('editPaymentForm').addEventListener('submit', function() {
                ...
                if (idField.value) {
                    this.action = "{{ route('payment.update', ':id') }}".replace(':id', idField.value);
                }
            });
            ...
            document.querySelectorAll('.deletePaymentBtn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    form.action = "{{ route('payment.delete', ':id') }}".replace(':id', id);
                });
            });
        });
    </script>
